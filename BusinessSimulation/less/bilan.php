                ...
                if ($didPlay==0){
                    ...
                    while ($donnees = $req->fetch())
                    {
                        ...
                        $req2->execute(array(
                                    ...
                                    'year' => $annee,
                                    'step' => 4,
                                    'nbActions' => $donnees["nbActions"]-$_POST["buyback"],
                        ...
                        while ($dette = $req10->fetch())
                        {
                            ...
                            $endett=$dette['value'] + $_POST["buyback"] + $_POST["dividende"]*$donnees['nbActions'];

                            $req15 = $bdd->prepare("UPDATE Dette SET value=:value WHERE player = ".$player." AND year = ".$annee." AND step = 3");
                            $req15->execute(array(
                                                'value' => $dette['value'] + $_POST["buyback"] + $_POST["dividende"]*$donnees['nbActions']
                                    ));
                            
                            echo "Endettement de : ".($_POST["buyback"] + $_POST["dividende"]*$donnees['nbActions'])." total : ".$endett;
                            ...
                            while ($ebit = $req20->fetch())
                            {
                                ...
                                $req21->execute(array(
                                                    'value' => $dette['value'] - $ebit['EBIT']*(0.7)
                                        ));
                                        echo "Second update ! EBIT : ".$ebit['EBIT']." et FCF : ".($ebit['EBIT']*0.7);
                            }
                        }
