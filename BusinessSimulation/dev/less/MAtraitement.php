...
// On créée les CA en step 3 qu'on actualisera ensuite
                            ...
                            foreach ($indices as $ind)
                            {
                                    ...
                                    while ($donnees = $req->fetch())
                                    {
                                        $req3 = $bdd->prepare('UPDATE Capital SET NbActions=:NbActions WHERE player='.$donnees['player'].' AND year = '.$annee.' AND step = 3');
                                        $req3->execute(array(
                                    ...
                                    while ($donnees = $req->fetch())
                                    {
                                        $req2 = $bdd->prepare('UPDATE Dette SET value=:value WHERE player='.$donnees['player'].' AND year = '.$donnees['year'].' AND step = 3');
                                        $req2->execute(array(
                                    ...
                                    $req = $bdd->query("SELECT NA_CS, EMEA_CS, ROW_CS, NA_NEW, EMEA_NEW, ROW_NEW, NA_OS, EMEA_OS, ROW_OS, NA_INFRA, EMEA_INFRA, ROW_INFRA, NA_BPO, EMEA_BPO, ROW_BPO FROM Positions WHERE year = ".$annee." AND step = 1 AND player = ".$i);
